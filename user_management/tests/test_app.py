...
class TestApp(unittest.TestCase):
    ...
    def _get_mock_token_headers(self):
        """Test için mock token header'ları oluştur"""
    ...
    def test_login_post_failure(self):
        ...
        self.assertIn('Geçersiz kullanıcı adı veya şifre'.encode('utf-8'), response.data)  # Hata mesajı içeriyor
    ...
    def test_products_list_with_token(self):
        ...
        response = self.app.get('/products', 
                               headers=self._get_mock_token_headers())
        self.assertEqual(response.status_code, 200)
...
class TestProductService(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_repo.find_all.return_value = [self.test_product]
        self.mock_repo.find_by_id.return_value = self.test_product
        
        # ProductService örneği oluştur
        from user_management.services.product_services import ProductService
        self.product_service = ProductService(self.mock_repo)
    ...
    def test_get_all_products(self):
        ...
        self.assertEqual(products[0]['name'], self.test_product['name'])
        self.mock_repo.find_all.assert_called_once()
    
    def test_get_product_by_id(self):
        """ID'ye göre ürün getirme testi"""
        product = self.product_service.get_product_by_id(str(self.test_product['_id']))
        self.assertEqual(product['name'], self.test_product['name'])
        self.mock_repo.find_by_id.assert_called_once_with(str(self.test_product['_id']))
